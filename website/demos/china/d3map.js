...
var g = svg.append('g');
var points = [];
for (taxon in CHINESE.groups) {
  
  var latlon = projection([CHINESE.coords[taxon][1], 
      CHINESE.coords[taxon][0]]);
  var _data = {
    "id" : taxon,
    "lon" : latlon[0],
    "lat" : latlon[1],
  ...
  g.selectAll('circle')
    ...
    .enter()
    .append('circle')
    .attr('id', function(d) {return 'taxon_'+d.name;})
    ...
    .attr('colorname', function(d){return CHINESE.conf.groups_colors[CHINESE.groups[d.name]];})
    .attr('class', 'mynode')
    .on('mouseover', function(d){show_alignment(d.name);})
    .on('mouseout', function(d){back2normal(d.name);})
    ;
    console.log(g);
